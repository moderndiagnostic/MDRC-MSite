...
export default function RadioLogyItem() {
  ...
  // Memoize the hook params to stabilize dependencies
  ...
  const filteredDiseases = useMemo(
    () =>
      Diseases.filter((disease: { name: string }) =>
  ...
  const filteredCategories = useMemo(
    () =>
      Category.filter((category: { name: string }) =>
  ...
  const allSelectedFilters = useMemo(
    ...
    [temporarySelectedDiseases, temporarySelectedCategories],
  );
  ...
  // const showBannerShimmer = initialLoading && banners.length === 0;
  ...
  // if (initialLoading && !hasFetchedOnce && bannerCache.current.length === 0) {
  //   return <p className="text-center py-6 text-gray-500">Loading...</p>;
  // }

  // if (!initialLoading && hasFetchedOnce && items.length === 0) {
  //   return <p className="text-center py-6 text-gray-500">No Items Found</p>;
  // }

  // ✅ Keep banners same after first fetch
  ...
  return (
    <div className="">
          ...
          <div className="fixed top-0 left-0 h-full w-[75%] bg-white z-50 transform transition-transform duration-300 ease-in-out overflow-y-auto translate-x-0">
            ...
            <div className="p-6">
              ...
              <button
                ...
                disabled={isPending}
                className={`w-full py-3 px-4 mb-24 rounded-md font-medium flex items-center justify-center gap-2 transition-all duration-200 ${
                  isPending
      ...
      <section className="my-8 px-4">
        <Link href="https://api.whatsapp.com/send/?phone=918586988847&text=Hello+%3A%29+Thank+you+for+contacting+Modern+Diagnostic+and+Research+Centre.+How+can+we+help+you+please%3F&type=phone_number&app_absent=0">
      ...
      {/* Additional sections */}
      <FaqAccordion pageType={"radiology"} />
