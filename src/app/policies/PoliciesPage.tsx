...
const PoliciesPage: React.FC<PoliciesPageProps> = ({ serverData }) => {
  const { documents, other, loading, error } = useIPO("Policies");
  const [activeQR, setActiveQR] = useState<string | null>(null);

  if (!serverData) return null;

  const handleDocumentClick = (doc: any) => {
    // ✅ If QR exists → show QR
    if (doc.qr) {
      setActiveQR(doc.qr);
    }

    // ✅ If file exists → download/open PDF
    if (doc.url) {
