...
export default function HealthTestsTabSlider() {
  ...
  const currentItems = useMemo(
    () =>
      homeData?.pathologyCatAndItem.find(
        (cat: HealthCategory) => cat.category_name === activeTab,
      )?.items || [],
    [activeTab],
  );

  const filtered = homeData?.pathologyCatAndItem.filter(
    (cat: HealthCategory) => cat.items?.length > 0,
  );

  if (filtered.length > 0 && !activeTab) {
    setActiveTab(filtered[0].category_name);
  }
  ...
  if (homeData?.pathologyCatAndItem?.length === 0) return null;
  ...
  return (
    <section className="mb-8 mt-2 px-4">
      ...
      <div className="flex gap-3 mb-4 overflow-x-auto no-scrollbar pb-2">
        {homeData?.pathologyCatAndItem?.map(
