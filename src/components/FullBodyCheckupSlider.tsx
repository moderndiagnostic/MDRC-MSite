...
export default function FullBodyCheckupSlider() {
  ...
  // AUTO SLIDE
  useEffect(() => {
    if (loading || homeData?.items?.length <= 1 || isPaused) return;
    intervalRef.current = setInterval(() => {
      setSlide((prev) => (prev + 1) % homeData?.items?.length);
    }, 5500);
    return () => {
      if (intervalRef.current) clearInterval(intervalRef.current);
    };
  }, [loading, homeData?.items?.length, isPaused]);
  ...
  const handleTouchEnd = () => {
    ...
    if (Math.abs(diff) > 40) {
      if (diff > 0) {
        ...
        setSlide(
          (prev) =>
            (prev - 1 + homeData?.items?.length) % homeData?.items?.length,
        );
      }
    }
  ...
  if (homeData?.items?.length === 0) return null;
